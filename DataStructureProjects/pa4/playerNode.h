...
public:
    ...
    // Program 4 Additions
    // Binary Tree Node node initialization
    playerNode* left = nullptr;
    playerNode* right = nullptr;


    // Methods for left and right node setting
    void setLeft(playerNode* node);
    void setRight(playerNode* node);

    // Methods for left and right node getting
    playerNode* getLeft();
    playerNode* getRight();
...
void playerNode::setLeft(playerNode* node) 
{
    left = node;
}

void playerNode::setRight(playerNode* node) 
{
    right = node;
}
